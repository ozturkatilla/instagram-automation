...
class SessionManager:
    ...
    def load_session(self, client: Client, username: str) -> bool:
        path = self.session_path(username)
        if not path.exists():
            logger.warning(f"Oturum dosyası bulunamadı: {username}")
            return False
        try:
            client.load_settings(path)
            logger.info(f"Oturum yüklendi: {username}")
            return True
        except Exception as e:
            logger.error(f"Oturum yüklenemedi {username}: {e}")
            return False
    ...
    def verify_session(self, client: Client) -> bool:
        try:
            ...
            logger.warning(f"Oturum doğrulama başarısız: {e}")
            return False